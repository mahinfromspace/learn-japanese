...
export function LibraryPage() {
  ...
  return (
    <div className="page library-page">
      ...
      {tab === 'kanji' && <div className="library-kanji-grid">{results.map((item) => { const status = progress.kanji.official.includes(item.id) ? 'Official' : progress.kanji.extra.includes(item.id) ? 'Extra' : ''; return <article key={item.id}><div className="library-card-top"><span className="big-character"><LinkedJapanese>{item.character}</LinkedJapanese></span>{status && <small>{status}</small>}</div><RevealStudyAids compact showJapanese={false} japanese={item.character} {...kanjiReadingSupport(item)} /><Link className="card-detail-link" to={`/kanji/${item.id}`}>Open detail →</Link></article>; })}</div>}
      {tab === 'vocabulary' && <div className="library-grid">{results.map((item) => <article key={item.id}><p className="eyebrow">{item.type}</p><h2><LinkedJapanese>{item.word}</LinkedJapanese></h2><RevealStudyAids compact showJapanese={false} japanese={item.word} hiragana={item.reading} romaji={item.romaji} meaning={item.meaning} /><Link className="card-detail-link" to={`/vocabulary/${item.id}`}>Open detail →</Link></article>)}</div>}
      {tab === 'grammar' && <div className="library-grid">{results.map((item) => <article key={item.id}><span className={`level-badge ${item.level.toLowerCase()}`}>{item.level}</span><h2><LinkedJapanese>{item.pattern}</LinkedJapanese></h2><RevealStudyAids compact showJapanese={false} japanese={item.pattern} romaji={item.romaji} meaning={item.meaning} /><Link className="card-detail-link" to={`/grammar/${item.id}`}>Open lesson →</Link></article>)}</div>}
