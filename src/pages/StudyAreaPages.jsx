  ...
  if (mode === 'daily' || mode === 'extra') return (
    <div className="page narrow-page">
      ...
      <FlashcardDeck
        ...
        renderBack={(item) => <><h2>{item.meaning}</h2><p><strong>On:</strong> {item.onyomi}</p><p><strong>Kun:</strong> {item.kunyomi}</p><p className="example-line"><LinkedJapanese>{item.word}</LinkedJapanese> · {item.wordReading} · {item.wordMeaning}</p><Link className="text-link" to={`/kanji/${item.id}`} state={{ returnTo: '/kanji' }} onClick={(event) => event.stopPropagation()}>Open full detail <ArrowRight /></Link></>}
      />
    </div>
  );
  ...
  return (
    <div className="page">
      ...
      <div className="item-preview-grid kanji-preview-grid">
        {daily.map((item) => <Link to={`/kanji/${item.id}`} state={{ returnTo: '/kanji' }} className={`preview-card ${progress.kanji.official.includes(item.id) ? 'is-learned' : ''}`} key={item.id}><span>{item.character}</span><strong>{item.meaning.split(';')[0]}</strong>{progress.kanji.official.includes(item.id) && <Check />}</Link>)}
      </div>
...
export function VocabularyPage() {
  ...
  if (studying) return (
    <div className="page narrow-page">
      ...
      <FlashcardDeck area="vocabulary" items={daily} onRate={(item, known) => { markLearned('vocabulary', item.id); review('vocabulary', item.id, known); }} onComplete={() => navigate('/test?area=vocabulary')} renderFront={(item) => <><div className="card-word"><LinkedJapanese>{item.word}</LinkedJapanese></div><p>{item.reading}</p></>} renderBack={(item) => <><h2>{item.meaning}</h2><p>{item.romaji} · {item.type}</p><p className="example-line"><LinkedJapanese>{item.examples?.[0]?.japanese || item.word}</LinkedJapanese></p><p>{item.examples?.[0]?.english || item.commonUsage}</p><Link className="text-link" to={`/vocabulary/${item.id}`} onClick={(event) => event.stopPropagation()}>Full word detail <ArrowRight /></Link></>} />
    </div>
  );
  return (
    <div className="page">
      ...
      <div className="word-preview-list">{daily.slice(0, 6).map((item) => <Link className="word-row" to={`/vocabulary/${item.id}`} key={item.id}><span className="word-main"><LinkedJapanese>{item.word}</LinkedJapanese></span><span>{item.reading}</span><strong>{item.meaning.split(';')[0]}</strong>{progress.vocabulary.learned.includes(item.id) && <Check />}</Link>)}</div>
...
export function GrammarPage() {
  ...
  if (studying) return (
    <div className="page narrow-page">
      ...
      <FlashcardDeck area="grammar" items={daily} onRate={(item, known) => { markLearned('grammar', item.id); review('grammar', item.id, known); }} onComplete={() => navigate('/test?area=grammar')} renderFront={(item) => <><span className={`level-badge ${item.level.toLowerCase()}`}>{item.level}</span><div className="card-pattern"><LinkedJapanese>{item.pattern}</LinkedJapanese></div><p>{item.romaji}</p></>} renderBack={(item) => <><h2>{item.meaning}</h2><p className="structure-box"><strong>Structure</strong><LinkedJapanese>{item.structure}</LinkedJapanese></p><p className="example-line"><LinkedJapanese>{item.examples?.[0]?.japanese || item.pattern}</LinkedJapanese></p><p>{item.examples?.[0]?.english || item.explanation}</p><Link className="text-link" to={`/grammar/${item.id}`} onClick={(event) => event.stopPropagation()}>Open full lesson <ArrowRight /></Link></>} />
    </div>
  );
  return (
    <div className="page">
      ...
      <div className="grammar-preview-grid">{daily.map((item) => <Link className="grammar-preview" to={`/grammar/${item.id}`} key={item.id}><span className={`level-badge ${item.level.toLowerCase()}`}>{item.level}</span><h3><LinkedJapanese>{item.pattern}</LinkedJapanese></h3><p>{item.meaning}</p>{progress.grammar.learned.includes(item.id) && <Check />}</Link>)}</div>
