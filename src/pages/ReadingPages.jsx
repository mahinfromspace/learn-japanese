  ...
  return (
    <div className="page">
      ...
      {tab === 'passages' ? <>
        ...
        <div className="reading-grid">{filtered.map((passage) => { const result = progress.reading.completed[passage.id]; return <Link className="reading-card" to={`/reading/${passage.id}`} key={passage.id}><div className="reading-card-top"><span className={`difficulty ${passage.difficulty.toLowerCase()}`}>{passage.difficulty}</span>{result && <span className="score-chip"><Check /> {result.score}/{result.total}</span>}</div><p className="eyebrow">{passage.type}</p><h2><LinkedJapanese>{passage.title}</LinkedJapanese></h2><div className="reading-meta"><span><Clock3 />{passage.minutes} min</span><span><ListChecks />{passage.questions.length} questions</span></div><span className="text-link">Open passage <ChevronRight /></span></Link>; })}</div>
      </> : <>
        ...
        <div className="sentence-library">{sentences.slice(0, 120).map((item) => <article key={item.id}><p className="japanese-sentence"><LinkedJapanese>{item.japanese}</LinkedJapanese></p><p>{item.english}</p><Link to={`/vocabulary/${item.word.id}`}>{item.word.word} · {item.word.meaning}</Link></article>)}</div>
      </>}
    </div>
  );
}
...
export function ReadingDetail() {
  ...
  return (
    <div className="page reading-detail-page">
      ...
      <header className="page-heading compact"><p className="eyebrow">{passage.type} · {passage.difficulty}</p><h1><LinkedJapanese>{passage.title}</LinkedJapanese></h1><div className="reading-meta"><span><Clock3 />{passage.minutes} min</span>{result && <span><Check />Best: {result.score}/{result.total}</span>}</div></header>
      <section className="passage-paper">
        <div className="paper-tools"><span>Japanese text</span><div><button className="icon-text-button" type="button" onClick={() => setRomaji((value) => !value)}><Languages /> {romaji ? 'Hide romaji' : 'Romaji'}</button><button className="icon-text-button" type="button" onClick={speak}><Volume2 /> Listen</button></div></div>
        <div className="passage-japanese"><LinkedJapanese>{passage.japanese}</LinkedJapanese></div>
        {romaji && <div className="romaji-panel">{passage.romaji}</div>}
        <button className="button secondary reveal-button" type="button" onClick={() => setTranslation((value) => !value)}>{translation ? <EyeOff /> : <Eye />}{translation ? 'Hide translation' : 'Reveal translation'}</button>
        {translation && <div className="translation-panel"><p className="eyebrow">ENGLISH</p><p>{passage.translation}</p></div>}
      </section>
      ...
      <section className="reading-quiz"><p className="eyebrow">COMPREHENSION</p><h2>Answer from the Japanese text</h2>{passage.questions.map((question, index) => <article className="reading-question" key={question.prompt}><h3><span>{index + 1}</span><LinkedJapanese>{question.prompt}</LinkedJapanese></h3><div className="quiz-options">{question.options.map((option, optionIndex) => { const selected = answers[index] === optionIndex; const state = submitted ? optionIndex === question.answer ? 'correct' : selected ? 'wrong' : 'muted' : selected ? 'selected' : ''; return <button type="button" className={state} disabled={submitted} key={option} onClick={() => setAnswers((current) => ({ ...current, [index]: optionIndex }))}><LinkedJapanese>{option}</LinkedJapanese>{submitted && optionIndex === question.answer && <Check />}</button>; })}</div>{submitted && <div className="answer-explanation"><strong>{answers[index] === question.answer ? 'Correct' : 'Review this one'}</strong><p>{question.explanation}</p></div>}</article>)}{!submitted ? <button className="button primary large" type="button" disabled={Object.keys(answers).length < passage.questions.length} onClick={submit}>Check answers</button> : <div className="result-inline"><strong>{score}/{passage.questions.length}</strong><span>{score === passage.questions.length ? 'Excellent comprehension.' : 'Read once more and locate the evidence for each answer.'}</span></div>}</section>
    </div>
  );
}
