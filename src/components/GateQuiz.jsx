  ...
  return (
    <div className="gate-backdrop">
      <section className="gate-card" role="dialog" aria-modal="true" aria-labelledby="gate-title">
        ...
        <div className="quiz-options">{question.options.map((option) => { const state = selected ? option === question.answer ? 'correct' : option === selected ? 'wrong' : 'muted' : ''; return <button type="button" className={state} key={option} onClick={() => { if (!selected) { setSelected(option); recordQuiz(option === question.answer ? 1 : 0, 1); } }}><LinkedJapanese>{option}</LinkedJapanese>{state === 'correct' && <Check />}{state === 'wrong' && <X />}</button>; })}</div>
